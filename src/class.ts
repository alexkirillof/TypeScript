...
import { IFindPlacesParams, IPlaces, IRequestParams } from './interfaces';

export abstract class FindPlaces { 
  private static async fetchHomeApi(requestParams: IRequestParams): Promise<IPlaces[]> {
    if (requestParams.method === 'GET') {
      const fetchURL = 'http://127.0.0.1:3001' + requestParams.endPoint + this.serializeToGetParams(requestParams.parameters)
      const response = await fetch(fetchURL)
      return await response.json()
    } else { 
      const fetchURL = 'http://127.0.0.1:3001' + requestParams.endPoint
      const response = await fetch(fetchURL, {
